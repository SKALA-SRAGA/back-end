...
def run_stt_stream(audio_queue, response_queue, language_code):
    ...
    try:
        ...
        # while True:  # 스트리밍 세션을 재시작할 수 있도록 루프 추가
        #     # 연결 시작 시간 기록
        ...
        #         yield config_request
        #         while True:
        #             # 연결이 오래되면 재연결하기 위해 타임아웃 체크 (예를 들어 270초 후)
        #             if time.time() - stream_start_time > 295:
        #                 # 인위적으로 None을 보내서 현재 스트림을 종료
        #                 yield None
        #                 break
        ...
        #         for response in responses:
        #             # 연결이 종료되어 responses 루프가 끝나면 새로운 스트림을 시도하게 됨
        #             if not response.results:
        #                 continue
        #             result = response.results[-1]
        ...
        #         logging.error(f"스트림 예외 발생: {stream_exception}")
        #         # 예외 발생 시 잠깐 대기 후 재시작
        #         time.sleep(1)
            
        #     # 현재 스트림 루프 종료 시 처리
        #     # 재연결을 시도하기 전에 오디오 큐의 상태나 연결 종료 신호를 확인하는 로직 추가 가능
        #     if audio_queue.empty():
        #         # 만약 오디오 큐가 비어 있다면 잠시 대기 후 재시작
        #         time.sleep(0.5)
